    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex items-start gap-2 mb-3">
          <h2 className="flex-1 text-[20px] font-bold leading-snug text-primary">{p.title}</h2>
          <span className="text-[13px] tabular-nums shrink-0 mt-1 text-tertiary">{p.year}</span>
        </div>
      ...
      {/* Image gallery */}
      <ProjectGallery project={p} galleryLabel={t.ui.gallery} />
      ...
      <div
        ...
        style={{ border: '0.5px solid var(--ios-separator)' }}
      >
        <p className="text-[15px] leading-relaxed text-secondary">{p.description}</p>
        {p.bullets.length > 0 && (
          <div className="space-y-2 pt-1">
            {p.bullets.map((b, i) => (
              <div key={i} className="flex gap-2">
                <span className="shrink-0 mt-[5px] text-[9px] text-system-blue">●</span>
          ...
          <div className="flex flex-wrap gap-1.5">
            {p.repos.map((repo) => (
              <span
                ...
                style={{ background: 'var(--ios-bg)', border: '0.5px solid var(--ios-separator)' }}
              >
                {repo}
              </span>
            ))}
